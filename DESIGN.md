...
- **Sidebar**: workspace switcher/profile menu → nav (Me / Team / Sessions / Stack) → active-stack pills → recent sessions → daemon footer with green pulse dot. The profile menu shows the user's name or email address when available, falls back to `Account`, and never exposes a raw UUID or WorkOS `user_...` subject. Nav item active state: `--selected` bg, t1 text, accent icon, font-weight 500.
