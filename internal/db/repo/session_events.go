...
func AppendSessionEvent(ctx context.Context, tx pgx.Tx, ev SessionEventRow) (SessionEventRow, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		RETURNING id, session_id, tenant_id, event_type, payload, occurred_at
	`, ev.TenantID, ev.SessionID, string(ev.EventType), payloadJSON, occurredArg).Scan(
		&out.ID, &out.SessionID, &out.TenantID, &out.EventType, &out.Payload, &out.OccurredAt,
	)
	if err != nil {
...
func scanEvents(rows pgx.Rows) ([]SessionEventRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ev.ID, &ev.SessionID, &ev.TenantID, &ev.EventType, &ev.Payload, &ev.OccurredAt); err != nil {
