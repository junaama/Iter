...
- **Signals derived in v1** (issue 011): `internal/signals.Aggregate` populates `peer_reuse_count`, `self_reuse_count`, `override_rate` (overrides / turn_completed, clamped to `[0,1]`), and `suggestion_acceptance` (accepted / (accepted + rejected)). `durability_7d` and `durability_30d` are NOT populated by per-session aggregation — they require windowed analysis across sessions and remain the nightly Modal batch's responsibility. A signal is left as a `nil` pointer when its denominator is zero (no turns ⇒ no override_rate; no suggestion outcomes ⇒ no acceptance). Counters of zero are also nil; a present-but-zero pointer would be indistinguishable from a real observation in downstream aggregations.
